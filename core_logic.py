...
def export_comparison_pdf_report(df_comparison, comparison_config, pdf_file_path, comparison_mode, logo_path):
    ...
    def create_comparison_chart(df, mode, title, x_label, y_label, img_path, comparison_config_inner):
        ...
        if mode in ["So Sánh Dự Án Trong Một Tháng", "Compare Projects in a Month"]:
            ...
            ax.set_xticklabels(df_plot['Project name'], rotation=45, ha="right")
        elif mode in ["So Sánh Dự Án Trong Một Năm", "Compare Projects in a Year"]:
            # df_plot.set_index('Project Name', inplace=True)
            # month_cols = [col for col in df_plot.columns if col not in ['Total Hours']]
            # df_plot[month_cols].T.plot(kind='line', marker='o', ax=ax)
            # ax.set_xlabel(x_label)
            # ax.set_ylabel(y_label)
            # ax.legend(title="Project Name", bbox_to_anchor=(1.05, 1), loc='upper left')

            df_plot_indexed = df_plot.set_index('Project Name')
            month_columns = [col for col in df_plot_indexed.columns if col in ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']]
            
            if not month_columns:
                print("DEBUG: No month columns found for 'Compare Projects in a Year' mode.")
                plt.close(fig)
                return None

            df_plot_indexed[month_columns].T.plot(kind='line', marker='o', ax=ax)
            ax.set_xlabel(x_label)
            ax.set_ylabel(y_label)
            ax.legend(title="Project Name", bbox_to_anchor=(1.05, 1), loc='upper left')

        elif mode in ["So Sánh Một Dự Án Qua Các Tháng/Năm", "Compare One Project Over Time (Months/Years)"]:
            if 'MonthName' in df_plot.columns: # So sánh theo tháng trong 1 năm
                df_plot.plot(kind='line', x='MonthName', y=df_plot.columns[1], marker='o', ax=ax, color='purple')
                ax.set_xlabel(x_label)
                ax.set_ylabel(y_label)
                # Đặt lại nhãn trục X theo thứ tự tháng
                month_order = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
                ax.set_xticks(range(len(df_plot['MonthName'])))
                ax.set_xticklabels(df_plot['MonthName'], rotation=45, ha="right")
            elif 'Year' in df_plot.columns: # So sánh theo năm
                df_plot.plot(kind='line', x='Year', y=df_plot.columns[1], marker='o', ax=ax, color='red')
                ax.set_xlabel(x_label)
                ax.set_ylabel(y_label)
            else:
                print(f"DEBUG: Invalid columns for 'Compare One Project Over Time' chart: {df_plot.columns}")
                plt.close(fig)
                return None
        else:
            print(f"DEBUG: Unknown comparison mode: {mode}")
            plt.close(fig)
            return None
    ...
    try:
        config_info = {
            ...
            "Years": ', '.join(map(str, comparison_config.get('years', []))) if comparison_config.get('years') else "All",
            "Months": ', '.join(comparison_config.get('months', [])) if comparison_config.get('months') else "All",
            "Projects": ', '.join(comparison_config.get('selected_projects', [])) if comparison_config.get('selected_projects') else "All"
        }
        ...
        if comparison_mode in ["So Sánh Dự Án Trong Một Tháng", "Compare Projects in a Month"]:
            if df_comparison.empty:
                ...
                chart_path = create_comparison_chart(df_comparison, comparison_mode, 
                                                     f"So sánh giờ giữa các dự án trong {comparison_config['months'][0]}, năm {comparison_config['years'][0]}",
                                                     "Dự án", "Giờ", os.path.join(tmp_dir, "comp_proj_month.png"), comparison_config)
            if chart_path:
                charts_for_pdf.append((chart_path, f"So sánh giờ giữa các dự án trong {comparison_config['months'][0]}, năm {comparison_config['years'][0]}", None))
            ...
            if df_comparison.empty:
                ...
                chart_path = create_comparison_chart(df_comparison, comparison_mode, 
                                                     f"So sánh giờ giữa các dự án trong năm {comparison_config['years'][0]} (theo tháng)",
                                                     "Tháng", "Giờ", os.path.join(tmp_dir, "comp_proj_year.png"), comparison_config)
            if chart_path:
                charts_for_pdf.append((chart_path, f"So sánh giờ giữa các dự án trong năm {comparison_config['years'][0]} (theo tháng)", None))
            ...
            if df_comparison.empty:
                ...
                chart_path = create_comparison_chart(df_comparison, comparison_mode, 
                                                     f"Tổng giờ dự án {selected_project_name} qua các tháng trong năm {comparison_config['years'][0]}",
                                                     "Tháng", "Giờ", os.path.join(tmp_dir, f"{sanitize_filename(selected_project_name)}_months.png"), comparison_config)
            elif 'Year' in df_comparison.columns:
                chart_path = create_comparison_chart(df_comparison, comparison_mode, 
                                                     f"Tổng giờ dự án {selected_project_name} qua các năm",
                                                     "Năm", "Giờ", os.path.join(tmp_dir, f"{sanitize_filename(selected_project_name)}_years.png"), comparison_config)
            if chart_path:
                charts_for_pdf.append((chart_path, f"Tổng giờ dự án {selected_project_name} qua thời gian", selected_project_name))
